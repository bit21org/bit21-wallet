...
- **Passphrase (25th word)** — plausible deniability for high-balance setups. Decoy wallet visible if coerced; real stash hidden behind passphrase.
...
- **No accounts, no KYC, no analytics** — we literally cannot see or track you. There is nothing to leak, subpoena, or breach.
- **Open-source and audit-friendly** — every line of wallet code is on GitHub. Reproducible builds coming soon.
...
The process is the same wherever your coins live today — an exchange, a custodial wallet, or a custodial app. You withdraw to a bit21 address, it arrives, you own it.
...
5. Wait for at least 1 on-chain confirmation (usually 10–30 minutes). Your Bitcoin is now in self-custody.
...
- **No tracking, no analytics, no telemetry** — we literally don't know who our users are
...
1. On the **Review** screen, check the recipient address character-by-character against the address you copied. Malware can swap a clipboard — your eyes are the last line of defence.
...
Your Bitcoin is the same. bit21 is just a different window into the same UTXOs. No on-chain fee, no waiting for confirmation.
...
open-source and welcomes independent review. Reproducible build
verification via [walletscrutiny.com](https://walletscrutiny.com) is
planned.
